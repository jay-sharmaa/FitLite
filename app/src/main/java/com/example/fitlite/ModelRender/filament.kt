package com.example.fitlite.ModelRender
...
class FilamentView(context: Context, private val fileName: String): SurfaceView(context), SurfaceHolder.Callback, Choreographer.FrameCallback{
    ...
    override fun surfaceCreated(holder: SurfaceHolder) {
        try {
            ...
            view.scene = scene
            setupLighting()
            loadGlbModel(filePath = myList[fileName.toInt()].obj)
    ...
    private fun loadGlbModel(filePath: String) {
        Thread {
            try {
                val bytes: ByteArray = if (filePath.startsWith("http")) {
                    val url = URL(filePath)
                    url.openConnection().getInputStream().use { it.readBytes() }
                } else {
                    context.assets.open(filePath).use { it.readBytes() }
                }
                    ...
                    post {
                        try {
                            ...
                            if (filamentAsset != null) {
                                ...
                                val transformManager = engine.transformManager
                                val root = filamentAsset!!.root
                                val transformInstance = transformManager.getInstance(root)

                                if (transformInstance != 0) {
                                    val transformMatrix = FloatArray(16)
                                    android.opengl.Matrix.setIdentityM(transformMatrix, 0)
                                    android.opengl.Matrix.translateM(transformMatrix, 0, 0f, -1.0f, 0f) // Shift down by 1 unit in Y

                                    transformManager.setTransform(transformInstance, transformMatrix)
                                }

                                Log.d(TAG, "Model loaded and translated downward.")
                            } else {
                                Log.e(TAG, "Filament asset creation failed.")
                            }
                        } catch (e: Exception) {
                            Log.e(TAG, "Error in render thread: ${e.message}", e)
                        }
                    }
                }

            } catch (e: Exception) {
                Log.e(TAG, "GLB download/load error: ${e.message}", e)
            }
        }.start()
    }
